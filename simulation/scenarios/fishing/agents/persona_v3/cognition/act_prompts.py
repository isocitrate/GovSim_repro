...
def prompt_action_choose_punishment_points(
    ...
    lm = model.start_chain(identity.name, "fishing_cognition_act", "punishment_points")
    ...
    with user():
        ...
        lm += (
            ...
            f"who caught {target_harvest} tons of fish this month. Each point costs "
            f"you {cost_per_point} payoff units and reduces {target_name}'s payoff "
            f"by {penalty_multiplier} payoff units. Choose an integer from 0 to "
            f"{max_points}. "
        )
        lm += reasoning_steps_prompt()
