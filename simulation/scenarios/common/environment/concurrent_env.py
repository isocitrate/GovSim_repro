...
class ConcurrentEnv:
    def __init__(
        ...
        self.PUNISHMENT_LOCATION = "punishment"
    ...
    def _prompt_punishment_report(self):
        report = []
        for agent in self.agents:
            agent_name = self.agent_id_to_name[agent]
            wanted = self.internal_global_state["wanted_resource"][agent]
            collected = self.internal_global_state["last_collected_resource"][agent]
            report.append(f"{agent_name} requested {wanted} and received {collected}.")

        cost = self.cfg.punishment.cost_per_point
        multiplier = self.cfg.punishment.penalty_multiplier
        max_points = self.cfg.punishment.max_points_per_target
        return (
            "Punishment stage. Each person may assign punishment points to other "
            "people. You cannot punish yourself. Each punishment point costs the "
            f"punisher {cost} payoff units and reduces the target's payoff by "
            f"{multiplier} payoff units. You may assign between 0 and {max_points} "
            f"points to each other person. Harvest report: {' '.join(report)}"
        )
    ...
    def _observe_punishment(self, agent) -> HarvestingObs:
        state = HarvestingObs(
            phase=self.phase,
            current_location=self.internal_global_state["next_location"][agent],
            current_location_agents=self.internal_global_state["next_location"],
            ...
            events=[
                PersonaEvent(
                    self._prompt_punishment_report(),
                    created=self.internal_global_state["next_time"][agent],
                    expiration=get_expiration_next_month(
                        self.internal_global_state["next_time"][agent]
                    ),
    ...
    def _observe(self, agent) -> HarvestingObs:
        ...
        elif self.phase == self.PUNISHMENT_LOCATION:
            state = self._observe_punishment(agent)
    ...
    def _init_agent(self, agent):
        ...
        self.internal_global_state["punishment_cost"][agent] = 0.0
        self.internal_global_state["punishment_penalty"][agent] = 0.0
    ...
    def reset(self, seed=None, options=None) -> tuple[str, HarvestingObs]:
        ...
        self._phase_selector = agent_selector(self._phase_order())
        self.phase = self._phase_selector.next()

        return self.agent_selection, self._observe(self.agent_selection)

    def _phase_order(self):
        phases = [self.POOL_LOCATION, "pool_after_harvesting", "restaurant"]
        if self._punishment_enabled():
            phases.append(self.PUNISHMENT_LOCATION)
        phases.append("home")
        return phases

    def _punishment_enabled(self):
        if hasattr(self.cfg, "get"):
            return bool(self.cfg.get("enable_punishment", False))
        return bool(getattr(self.cfg, "enable_punishment", False))
    ...
    def _step_punishment(self, action: PersonaActionPunishment):
        self.internal_global_state["action"][self.agent_selection] = action
        self.internal_global_state["next_location"][
            self.agent_selection
        ] = self.PUNISHMENT_LOCATION
        if self._agent_selector.is_last():
            self._apply_punishments()
            for a in self.agents:
                self.internal_global_state["next_location"][a] = "home"
                self.internal_global_state["next_time"][a] = get_reflection_day(
                    self.internal_global_state["next_time"][a]
    ...
    def _apply_punishments(self):
        max_points = int(self.cfg.punishment.max_points_per_target)
        cost_per_point = float(self.cfg.punishment.cost_per_point)
        penalty_multiplier = float(self.cfg.punishment.penalty_multiplier)

        for agent in self.agents:
            self.internal_global_state["punishment_cost"][agent] = 0.0
            self.internal_global_state["punishment_penalty"][agent] = 0.0

        for punisher in self.agents:
            action = self.internal_global_state["action"].get(punisher)
            if not isinstance(action, PersonaActionPunishment):
                continue
            for target in self.agents:
                if target == punisher:
                    continue
                raw_points = action.punishment_points.get(target, 0)
                points = int(max(0, min(max_points, raw_points)))
                cost = points * cost_per_point
                penalty = points * penalty_multiplier
                self.internal_global_state["punishment_cost"][punisher] += cost
                self.internal_global_state["punishment_penalty"][target] += penalty
                self.rewards[punisher] -= cost
                self.rewards[target] -= penalty
                self.log_step_punishment(action, target, points, cost, penalty)
    ...
    def step(self, action: PersonaAction) -> tuple[str, HarvestingObs, dict, dict]:
        ...
        if self.phase == self.POOL_LOCATION:
            ...
            assert action.location == self.PUNISHMENT_LOCATION
            assert type(action) == PersonaActionPunishment
            self._step_punishment(action)
        elif self.phase == "home":
    ...
    def log_step_punishment(
        self,
        action: PersonaActionPunishment,
        target_agent_id: str,
        ...
        df_log = pd.DataFrame(
            {
                "agent_id": action.agent_id,
                "punisher_agent_id": action.agent_id,
