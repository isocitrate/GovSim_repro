...
class SheepPersona(PersonaAgent):
    ...
    def loop(self, obs: HarvestingObs) -> PersonaAction:
        ...
        if obs.current_location == "pasture" and obs.phase == "pasture":
        ...
        elif obs.current_location == "punishment":
            retireved_memory = self.retrieve.retrieve([obs.current_location], 10)
            target_resources = {
                agent_id: resource
                for agent_id, resource in obs.agent_resource_num.items()
                if agent_id != self.agent_id
            }
            punishment_points, html_interactions = self.act.choose_punishment_points(
                retireved_memory,
                obs.current_location,
                obs.current_time,
                obs.context,
                target_resources,
                self.cfg.punishment.max_points_per_target,
                self.cfg.punishment.cost_per_point,
                self.cfg.punishment.penalty_multiplier,
            )
            action = PersonaActionPunishment(
                ...
                punishment_points,
                stats={
                    f"{self.agent_id}_punishment_points": sum(
                        punishment_points.values()
                    )
                },
                html_interactions=html_interactions,
            )
