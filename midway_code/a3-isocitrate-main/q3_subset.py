...
CV_TRAIN_FRACTION = 0.15

def main():
    spark = create_spark("macs30123_a3_q3")
    df = load_or_prepare_model_df(spark)
    train_df, test_df = split_train_test(df)
    cv_train_df = train_df.sample(False, CV_TRAIN_FRACTION, seed=SEED)
    ...
    cv_model = cv.fit(cv_train_df)
    cv_best_model = cv_model.bestModel
    cv_lr_model = cv_best_model.stages[-1]

    final_pipeline = build_pipeline(
        reg_param=cv_lr_model.getRegParam(),
        elastic_net_param=cv_lr_model.getElasticNetParam(),
    )
    ...
    with summary_path.open("w") as f:
        ...
        f.write(f"CV training subset fraction: {CV_TRAIN_FRACTION}\n")
        f.write(f"RMSE: {rmse}\n")
    ...
    print("\nQ3 results:")
    print(f"CV training subset fraction: {CV_TRAIN_FRACTION}")
    print(f"RMSE: {rmse}")
